...
def train_model_process(model, train_dataloader, val_dataloader, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for step, (b_x, b_y) in enumerate(tqdm(val_dataloader, desc=f"Epoch {epoch} valing")):
            # 将特征放入到验证设备中
            b_x = b_x.to(device)
            # 将标签放入到验证设备中
            b_y = b_y.to(device)
    ...
    torch.save(best_model_wts, "/Users/xuehaoyu/Desktop/learnAI/pytorch_pao/code/ResNet18-Masks/best_model.pth")
...
if __name__ == '__main__':
    ...
    # 利用现有的模型进行模型的训练
    train_process = train_model_process(ResNet18, train_data, val_data, num_epochs=2)
